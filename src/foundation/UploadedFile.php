...
{
    protected const ERROR_MOVE_DONE = 'ERROR: file already moved';
    protected const ERROR_BAD_FILE = 'ERROR: no uploaded file';
    protected const ERROR_BAD_DIR = 'ERROR: directory does not exist';
    protected const ERROR_FILE_NOT = 'ERROR: file is not an uploaded file';
    protected const ERROR_MOVE_UNABLE = 'ERROR: unable to move file';
    ...
    {
        if ($this->moved) {
            throw new Exception(self::ERROR_MOVE_DONE);
        }
        if (!file_exists($targetPath)) {
            throw new InvalidArgumentException(self::ERROR_BAD_DIR);
        }
        $tempFile = $this->info['tmp_name'] ?? FALSE;
        if (!$tempFile || !file_exists($tempFile)) {
            throw new Exception(self::ERROR_BAD_FILE);
        }
        if (!is_uploaded_file($tempFile)) {
            throw new Exception(self::ERROR_FILE_NOT);
        }
        if ($this->randomize) {
        ...
        if (!move_uploaded_file($tempFile, $final)) {
            throw new RuntimeException(self::ERROR_MOVE_UNABLE);
        }
