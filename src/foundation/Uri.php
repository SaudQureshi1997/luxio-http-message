...
{
    protected const ERROR_INVALID_URI = 'ERROR: invalid URI';
    ...
    public function __construct($uriString = NULL)
    {
        if ($uriString) {
            ...
            if (!$this->uriParts) {
                throw new InvalidArgumentException(self::ERROR_INVALID_URI);
            }
    ...
    public function getPort()
    {
        if (empty($this->uriParts['port'])) {
            ...
            if ($this->getScheme()) {
                if ($this->uriParts['port'] == self::STANDARD_PORTS[$this->getScheme()]) {
                    return NULL;
                }
            }
    ...
    public function getPath()
    {
        ...
        return implode(
            '/',
            array_map("rawurlencode", explode('/', $this->uriParts['path']))
        );
    }
    ...
    /**
     * Not defined by PSR-7
     * Returns query params in the form of an associative array
     *
     * @param bool $reset = TRUE = refresh query params
    ...
    public function withScheme($scheme)
    {
        if (empty($scheme) && $this->getScheme()) {
            ...
            if (isset(self::STANDARD_PORTS[strtolower($scheme)])) {
                $this->uriParts['scheme'] = $scheme;
            } else {
                throw new InvalidArgumentException(self::ERROR_BAD . __METHOD__);
            }
        }
        return $this;
    }
