...
{
    protected const DEFAULT_REQUEST_TARGET = '/';
    protected const HTTP_METHODS  = ['get', 'put', 'post', 'delete'];
    protected const ERROR_HTTP_METHOD = 'ERROR: invalid HTTP method';
    protected const HEADER_HOST   = 'Host';     // host header
    ...
    {
        return $this->uri ?? self::DEFAULT_REQUEST_TARGET;
    }
    ...
    {
        if (!$method === NULL) {
            if (!in_array(strtolower($method), self::HTTP_METHODS)) {
                throw new InvalidArgumentException(self::ERROR_HTTP_METHOD);
            }
        }
        return $method;
    }
    ...
    {
        if ($preserveHost) {
            $found = $this->findHeader(self::HEADER_HOST);
            if (!$found && $uri->getHost()) {
                $this->httpHeaders[self::HEADER_HOST] = $uri->getHost();
            }
        } elseif ($uri->getHost()) {
            $this->httpHeaders[self::HEADER_HOST] = $uri->getHost();
        }
